...
# 메인 루프
try:
    print("W: 앞으로, S: 뒤로, X: 정지, A: 서보 왼쪽, D: 서보 오른쪽")
    print("+: 속도 증가, -: 속도 감소, C: 서보 중앙, E: 종료")
    ...
    while True:
        ...
        if key == 'w': control_motor("forward", speed)
        elif key == 's': control_motor("backward", speed)
        elif key == 'x': control_motor("stop")
        elif key == '+': speed = min(100, speed + 5)
        elif key == '-': speed = max(0, speed - 5)
        elif key == 'a': angle = max(30, angle - 5); control_servo(angle)
        elif key == 'd': angle = min(80, angle + 5); control_servo(angle)
        elif key == 'c': angle = 55; control_servo(angle)
        elif key == 'e': break
