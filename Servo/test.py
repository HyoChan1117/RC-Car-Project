...
def servo_control(angle):
    # Convert the angle (30-80 degrees) to PWM duty cycle
    duty_cycle = 500 + ((angle - 30) / 50) * 2000  # Adjust duty cycle based on new angle range (30-80)
    lgpio.tx_servo(h, SERVO_PIN, int(duty_cycle))
    print(f"Servo moved to {angle} degrees")

# Function to continuously capture and save images
def capture_images():
    desktop_path = "/home/pi/Desktop/captured_images"  # Path to save images in 'captured_images' folder
...
try:
    ...
    print("A - Servo Left, D - Servo Right, X - Stop Motor, E - Quit, C - Center Servo")

    # Define center angle for the servo
    center_angle = 55  # Set the initial center position (between 30 and 80 degrees)
    current_speed = 30  # Initial speed (0 to 100)
    current_direction = "stop"  # Initial direction
    servo_angle = center_angle  # Start servo at center position
    ...
    while True:
        ...
        # Servo Motor Controls
            ...
            servo_angle = max(30, servo_angle - 5)  # Decrease angle but not below 30
            servo_control(servo_angle)
        elif key == 'd':  # Turn servo right
            servo_angle = min(80, servo_angle + 5)  # Increase angle but not above 80
            servo_control(servo_angle)
